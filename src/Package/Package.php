...
class Package
{
    ...
    public function __construct(string $packageArchiveFilePath)
    {
        ...
        if ($packageDescriptorData === FALSE) {
            throw new Exception("OMM Package Descriptor (" . self::PACKAGE_DESCRIPTOR_FILE . ") not found or invalid in" . $packageArchiveFilePath);
        }
        ...
        if (!empty($logoImageName)) {
            ...
            if ($this->logoImageData === FALSE) {
                throw new Exception("OMM Package Logo (" . $logoImageName . ") not found or invalid in " . $packageArchiveFilePath);
            }
