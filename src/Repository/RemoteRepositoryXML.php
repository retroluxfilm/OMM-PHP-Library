...
class RemoteRepositoryXML
{
    ...
    protected function updateRepositoryFromXMLData(string $repositoryTitle, string $repositoryRootPath): void
    {
        ...
        if ($downPathList->length == 0) {
            $downpath = $this->xml->createElement(self::TAG_DOWNPATH, RepositoryHelper::urlEncodePath($repositoryRootPath . DIRECTORY_SEPARATOR));
            $this->root->appendChild($downpath);
        } else {
            $downPathList->item(0)->nodeValue = RepositoryHelper::urlEncodePath($repositoryRootPath . DIRECTORY_SEPARATOR);
        }
